...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['submit'])){
        ...
        if (empty($fname)) {
            ...
            if (strlen($name) > 100) {
                $nameErr = "* Le nom doit comporter un maximum de 100 caractères.";
            }
        }
        if (empty($username)) {
		...
		<form id="login" action="inscription.php" method="post">
			<fieldset>
				...
				<table class="tab-insc" cellpadding="10" cellspacing="5">
					<tr> <td><label for="fname">First name:</label> </td> <td><input type="text" id="fname" name="fname" maxlength="100"  value="<?php echo htmlentities($fname);?>" required /> <span class="error"> <?php echo $fname;?></span> </td> </tr>
					...
					<tr><td><label for="password-confirmation">Password-confirmation:</label> </td> <td> <input type="password" id="password-confirmation" name="password-confirmation"value="<?php echo htmlentities($passwordConfirmation);?>" required /> <span class="error"> <?php echo $passwordConfirmation;?></span></td></tr>
				</table>
				<div class="btn-insc">
